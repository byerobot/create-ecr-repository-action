...
const runForECR = async (inputs) => {
    ...
    if (repositoryPolicy !== undefined) {
        await core.group(`Put the repository policy to repository ${inputs.repository}`, async () => await setRepositoryPolicy(client, inputs.repository, repositoryPolicy));
    }
    return {
...
const putLifecyclePolicy = async (client, repositoryName, lifecyclePolicyText) => {
...
const setRepositoryPolicy = async (client, repositoryName, policyText) => {
    core.debug(`putting the repository policy ${policyText} to repository ${repositoryName}`);
    await client.send(new client_ecr_1.SetRepositoryPolicyCommand({
        repositoryName,
        policyText,
    }));
...
exports.run = void 0;
const core = __importStar(__nccwpck_require__(2186));
const ecr_1 = __nccwpck_require__(2313);
